...
For information about what category your data falls in, see [Data Governance documentation](https://www.cu.edu/data-governance/resources-support/data-classification) page.

### External

Along with CU data classifications, some data types, such as [Health Insurance Portability and Accountability Act (HIPAA)](https://www.hhs.gov/hipaa/index.html) and [Payment Card Industry Data Security Standard (PCI DSS)](https://en.wikipedia.org/wiki/Payment_Card_Industry_Data_Security_Standard), are regulated by outside entities. This data must adhere to additional requirements and auditing above what CU requires internally.
...
| Data Classification | Supported on LCA1? |
| --- | --- |
| Internal - Public | Yes |
| Internal - Confidential | Yes |
| Internal - Highly Confidential | No |
| External | No |

```{note}
Even for supported data classifications, **the LCA1 platform alone does not satisfy all required security controls** to protect that data. You must implement additional controls within your workload. Please see [Shared Responsibility Model](shared-responsibility-model.md) for more information.
```