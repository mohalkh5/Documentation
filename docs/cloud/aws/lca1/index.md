...
This page describes how to access your CU Boulder AWS LCA1 environment through the AWS Management Console and AWS CLI.
...
2. Provide your [CU IdentiKey](https://oit.colorado.edu/services/identity-access-management/identikey) credentials.
    ```{image} lca1_images/aws-console-access/login.png
    :alt: CU SSO login page. Described under single sign-on.
    :align: center
    ```
...
5. You will be presented with a list of account names and numbers to which you have access. Click on the name of the desired account to expand the roles allocated to you. Click on the name of the desired role to log into the AWS console as that role. **Note:** Your access to the account selection page is valid for **8 hours**. Your access to any specific AWS account console is valid for **4 hours**.
    ```{image} lca1_images/aws-console-access/select-role.png
    :alt: AWS SSO account and role selection page. Described under Single Sign-On.
    :align: center
    ```
6. You will be logged in to the AWS Management Console. Always be sure to verify you have the AWS Region you are working with selected after logging in.
    ```{image} lca1_images/aws-console-access/select-region.png
    :alt: AWS Console home page with the region selection box circled in red. Described under Single Sign-On.
    :align: center
    ```
...
1. Log in to the [AWS Management Console (SSO URL)](https://aws.colorado.edu) using steps 1-4 from [Single Sign-On](#single-sign-on).
2. You will be presented with a list of account names and numbers to which you have access. Click on the name of the desired account to expand the roles allocated to you. Click on "Access Keys" next to the name of the desired role.
    ```{image} lca1_images/aws-cli-access/select-access-keys.png
...
3. You will be presented with multiple options for acquiring access keys for the CLI. Click on the tab that matches your operating system (OS).
    ```{image} lca1_images/aws-cli-access/browser-cli-os-selection.png
...
4. Expand the section named "Option 1: Set AWS environment variables" and click on the overlapping squares icon to copy the environment variable commands that contain your credentials. The exact commands displayed will vary by OS and shell.
    ```{image} lca1_images/aws-cli-access/browser-cli-option1.png
    :alt: AWS CLI credentials access page with the copy icon circled in red next to a list of environment variable commands. Described under Option 1: Environment variables from the web browser.
    :align: center
    ```
...
4. Expand the section named "Option 2: Add a profile to your AWS credentials file" and click on the overlapping squares icon to copy the configuration block that contain your credentials.
    ```{image} lca1_images/aws-cli-access/browser-cli-option2.png
...
3. Log into SSO.
    ...
    ```{image} lca1_images/aws-cli-access/authorize-request.png
    ...
    ```{image} lca1_images/aws-cli-access/allow-botocore.png
    ...
    ```{image} lca1_images/aws-cli-access/request-approved.png
...
If your confidential workload requires access to a Non-US region, submit a [support request form](https://colorado.service-now.com/req_portal?id=ucb_sc_rc_form) and we can help evaluate the feasibility.
